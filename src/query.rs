...
fn parse_select(tokens: &[&str]) -> Result<Query, String> {
    ...
    if i < tokens.len() && tokens[i].to_uppercase() == "WHERE" {
        ...
        while i < tokens.len() {
            ...
            if i + 2 >= tokens.len() {
                return Err("Incomplete condition".into());
            }
    ...
    if i < tokens.len() && tokens[i].to_uppercase() == "LIMIT" {
        ...
        limit = Some(
            tokens[i]
                .parse::<usize>()
                .map_err(|_| format!("Invalid LIMIT value: {}", tokens[i]))?,
        );
    }
...
fn parse_delete(tokens: &[&str]) -> Result<Query, String> {
    ...
    if conditions.is_empty() {
        return Err("DELETE WHERE requires at least one condition".into());
    }
    Ok(Query {
...
fn parse_insert(tokens: &[&str]) -> Result<Query, String> {
    // INSERT <key> <value...>
    if tokens.len() < 3 {
        return Err("INSERT requires <key> <value>".into());
    }
...
fn parse_update(tokens: &[&str]) -> Result<Query, String> {
    ...
    if tokens.len() < 8 {
        return Err("UPDATE requires SET value = <val> WHERE key = <key>".into());
    }
    ...
    if value_tokens.is_empty() {
        return Err("Missing value in UPDATE".into());
    }
    ...
    Ok(Query {
        action: Action::Update(key, value),
        conditions: Vec::new(),
        limit: None,
        order_desc: false,
    })
...
mod tests {
    ...
    fn test_select_limit_order() {
        let q =
            parse_query("SELECT * WHERE key >= 0 ORDER BY DESC LIMIT 10").expect("unwrap failed");
        assert_eq!(q.action, Action::SelectAll);
        assert!(q.order_desc);
    ...
    fn test_insert() {
        ...
        assert_eq!(
            q.action,
            Action::Insert("42".to_string(), "Hello World".to_string())
        );
    }
    ...
    fn test_update() {
        ...
        assert_eq!(
            q.action,
            Action::Update("123".to_string(), "new data".to_string())
        );
    }
