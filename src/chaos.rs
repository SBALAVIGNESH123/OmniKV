...
pub fn test_crash_recovery(manifest_path: &str, wal_path: &str) -> ChaosResult {
    ...
    let committed_keys: Vec<(String, String)> = {
        let db = match OmniKV::open(manifest_path, wal_path) {
            Ok(db) => db,
            Err(e) => return ChaosResult::fail("crash_recovery", 0, 1, format!("Failed to open: {}", e)),
        };
        ...
        for i in 0..iterations {
            ...
            if let Err(e) = batch.set(&key, value.clone()) {
                return ChaosResult::fail("crash_recovery", i, 1, format!("batch.set failed: {}", e));
            }
    ...
    let db = match OmniKV::open(manifest_path, wal_path) {
        Ok(db) => db,
        Err(e) => return ChaosResult::fail("crash_recovery", iterations, 1, format!("Reopen failed: {}", e)),
    };
    ...
    for (key, expected_value) in &committed_keys {
        match db.find(key, seq) {
            ...
            Ok(Some(val)) => {
                anomalies += 1;
                details.push(format!("CORRUPTION: key={} expected={} got={}", key, expected_value, val));
            }
            Ok(None) => {
                anomalies += 1;
                details.push(format!("DATA LOSS: key={} was committed but missing after recovery", key));
            }
            Err(e) => {
                anomalies += 1;
...
/// ═══════════════════════════════════════════════════════════════════════
pub fn test_concurrent_ww_conflicts(db: Arc<OmniKV>, num_threads: usize, rounds: u64) -> ChaosResult {
    ...
    for round in 0..rounds {
        ...
        if seed.set(&key, "initial".to_string()).is_err() { continue; }
        if db.commit_batch(&seed).is_err() { continue; }
        ...
        let results: Vec<Result<u64, OmniError>> = handles.into_iter()
        ...
        if commits.len() != 1 {
            ...
            details.push(format!(
                ...
                round, commits.len(), aborts.len(), num_threads - 1
            ));
        }
    }

    if anomalies > 0 {
        ChaosResult::fail("concurrent_ww_conflicts", rounds, anomalies, details.join("; "))
    } else {
...
pub fn test_write_skew_detection(db: Arc<OmniKV>) -> ChaosResult {
    ...
    for round in 0..rounds {
        ...
        if both_committed {
            ...
            if v1 == "off_call" && v2 == "off_call" {
                anomalies += 1;
                details.push(format!("Round {}: WRITE SKEW — both doctors off-call!", round));
            }
        }
    ...
    if anomalies > 0 {
        ChaosResult::fail("write_skew_detection", rounds, anomalies, details.join("; "))
    } else {
...
pub fn test_data_integrity(db: Arc<OmniKV>, num_keys: u64) -> ChaosResult {
    ...
    let test_data: Vec<(String, String)> = (0..num_keys).map(|i| {
            ...
            Ok(Some(actual)) => {
                if actual != *expected {
                    ...
                    details.push(format!(
                        ...
                        key, expected.len(), actual.len(), actual == *expected
                    ));
                }
            }
            Ok(None) => {
...
/// ═══════════════════════════════════════════════════════════════════════
pub fn test_monotonic_sequences(db: Arc<OmniKV>, num_threads: usize, ops_per_thread: u64) -> ChaosResult {
    let barrier = Arc::new(Barrier::new(num_threads));
    let mut handles = Vec::new();
                    ...
                    Ok(seq) => {
                        if seq <= prev_seq && prev_seq > 0 {
                            violations.push(format!(
                                "thread={} op={} prev_seq={} cur_seq={}", t, i, prev_seq, seq
                            ));
                        }
                        prev_seq = seq;
                    }
...
pub fn test_concurrent_stress(db: Arc<OmniKV>) -> ChaosResult {
    ...
    if errors.is_empty() {
        ...
        ChaosResult::fail("concurrent_stress", total, errors.len() as u64, errors.join("; "))
    }
}

/// Runs all chaos tests and returns a summary.
pub fn run_all_chaos_tests(db: Arc<OmniKV>, manifest_path: &str, wal_path: &str) -> Vec<ChaosResult> {
    vec![
