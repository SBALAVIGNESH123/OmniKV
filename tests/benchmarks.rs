...
fn create_bench_db() -> (Arc<OmniKV>, tempfile::TempDir) {
    ...
    let manifest = dir.path().join("bench_manifest").to_string_lossy().to_string();
...
impl BenchResult {
    fn print(&self) {
        if let Some(mb) = self.mb_per_sec {
            println!("  {:40} {:>10} ops in {:>6}ms = {:>12.0} ops/sec ({:.1} MB/s)",
                self.name, self.ops, self.elapsed_ms, self.ops_per_sec, mb);
        } else {
            println!("  {:40} {:>10} ops in {:>6}ms = {:>12.0} ops/sec",
...
fn bench_sequential_writes(db: &Arc<OmniKV>, count: u64) -> BenchResult {
    ...
    BenchResult {
        ...
        ops: count, elapsed_ms: elapsed.as_millis(),
        ops_per_sec, mb_per_sec: Some(mb_per_sec),
    }
}
...
fn bench_batch_writes(db: &Arc<OmniKV>, total_keys: u64, batch_size: u64) -> BenchResult {
    ...
    BenchResult {
        ...
        ops: written, elapsed_ms: elapsed.as_millis(),
        ops_per_sec, mb_per_sec: Some(mb_per_sec),
    }
}
...
fn bench_range_scan(db: &Arc<OmniKV>) -> BenchResult {
    ...
    BenchResult {
        ...
        ops: iterations as u64, elapsed_ms: elapsed.as_millis(),
        ops_per_sec: rows_per_sec, mb_per_sec: None,
    }
}
...
fn bench_mixed_workload(db: &Arc<OmniKV>, count: u64) -> BenchResult {
    ...
    BenchResult {
        ...
        ops, elapsed_ms: elapsed.as_millis(),
        ops_per_sec, mb_per_sec: None,
    }
}
