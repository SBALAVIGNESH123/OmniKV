...
use omni_engine::{OmniKV, WriteBatch, OmniError};
use std::sync::Arc;
...
fn create_test_db() -> (Arc<OmniKV>, TempDir) {
    ...
    let manifest = dir.path().join("test_manifest").to_string_lossy().to_string();
...
fn test_scan_range() {
    ...
    for i in 0..10 {
        let mut batch = WriteBatch::new();
        batch.set(&format!("scan_{:03}", i), format!("val_{}", i)).unwrap();
        db.commit_batch(&batch).unwrap();
    }
...
fn test_many_keys_stress() {
    ...
    for i in 0..count {
        let mut batch = WriteBatch::new();
        batch.set(&format!("stress_{:06}", i), format!("payload_{}", i)).unwrap();
        db.commit_batch(&batch).unwrap();
    }
...
fn test_compaction_l0() {
    ...
    for i in 0..100 {
        let mut batch = WriteBatch::new();
        batch.set(&format!("compact_{:04}", i), format!("v{}", i)).unwrap();
        db.commit_batch(&batch).unwrap();
    }
...
fn test_concurrent_writes() {
    ...
    let handles: Vec<_> = (0..4).map(|t| {
        ...
        std::thread::spawn(move || {
            for i in 0..100 {
                let mut batch = WriteBatch::new();
                batch.set(&format!("thread{}_{:04}", t, i), format!("val_{}_{}", t, i)).unwrap();
                db.commit_batch(&batch).unwrap();
            }
        })
    }).collect();
    ...
    assert!(found >= 360, "Only {} of 400 concurrent writes readable", found);
}
...
fn test_scan_after_overwrite() {
    ...
    let seq = db.get_seq();
    let results: Vec<_> = db.scan_iter("range_001", "range_002", seq).unwrap().collect();
...
fn test_txn_basic_commit() {
    ...
    let seq = db.get_seq();
    assert_eq!(db.find("tx_key1", seq).unwrap(), Some("tx_val1".to_string()));
    assert_eq!(db.find("tx_key2", seq).unwrap(), Some("tx_val2".to_string()));
}
...
fn test_txn_read_your_own_writes() {
    ...
    let mut txn = tm.begin();
    tm.set(&mut txn, "ryw_key", "written_in_txn".to_string()).unwrap();
...
fn test_txn_write_write_conflict() {
    ...
    let mut t2 = tm.begin();
    tm.set(&mut t2, "conflict_key", "t2_wins".to_string()).unwrap();
    tm.commit(&mut t2).unwrap();
    
    // T1 tries to write the same key — should CONFLICT
    tm.set(&mut t1, "conflict_key", "t1_loses".to_string()).unwrap();
    let result = tm.commit(&mut t1);
    
    assert!(result.is_err(), "Should detect write-write conflict");
    let err = result.unwrap_err().to_string();
    assert!(err.contains("SSI CONFLICT"), "Error should mention SSI CONFLICT: {}", err);
    
    // T2's write should be the final value
    let seq = db.get_seq();
    assert_eq!(db.find("conflict_key", seq).unwrap(), Some("t2_wins".to_string()));
}
...
fn test_txn_read_write_conflict() {
    ...
    tm.set(&mut t1, "other_key", "whatever".to_string()).unwrap();
    let result = tm.commit(&mut t1);
...
fn test_txn_abort() {
    ...
    let mut txn = tm.begin();
    tm.set(&mut txn, "abort_key", "should_not_exist".to_string()).unwrap();
    tm.abort(&mut txn);
...
use omni_engine::secondary_index::{IndexManager, IndexFieldType};
...
fn test_index_create_and_lookup() {
    ...
    let idx_id = im.create_index("users_email_idx", "users", vec![
        ("email".to_string(), IndexFieldType::String),
    ], false).unwrap();
    assert!(idx_id > 0);
    ...
    let mut batch = WriteBatch::new();
    batch.set("users:alice", serde_json::to_string(&doc).unwrap()).unwrap();
    im.index_document("users", "alice", &doc, &mut batch).unwrap();
    db.commit_batch(&batch).unwrap();
    
    // Lookup by email
    let results = im.lookup("users_email_idx", &[serde_json::json!("alice@example.com")]).unwrap();
...
fn test_index_range_scan_integers() {
    ...
    im.create_index("users_age_idx", "users", vec![
        ("age".to_string(), IndexFieldType::Integer),
    ], false).unwrap();
    
    // Insert users with various ages
    for (name, age) in &[("alice", 25), ("bob", 30), ("charlie", 35), ("diana", 20), ("eve", 40)] {
        let doc = serde_json::json!({"name": name, "age": age});
        let mut batch = WriteBatch::new();
        let pk = format!("users:{}", name);
        batch.set(&pk, serde_json::to_string(&doc).unwrap()).unwrap();
        im.index_document("users", name, &doc, &mut batch).unwrap();
        db.commit_batch(&batch).unwrap();
    }
    
    // Range scan: age 25 to 35
    let results = im.range_scan("users_age_idx", &serde_json::json!(25), &serde_json::json!(35)).unwrap();
    assert_eq!(results.len(), 3, "Should find alice(25), bob(30), charlie(35)");
}
...
fn test_index_unique_constraint() {
    ...
    im.create_index("users_email_unique", "users", vec![
        ("email".to_string(), IndexFieldType::String),
    ], true).unwrap();
    ...
    batch1.set("users:user1", serde_json::to_string(&doc1).unwrap()).unwrap();
    im.index_document("users", "user1", &doc1, &mut batch1).unwrap();
    ...
    batch2.set("users:user2", serde_json::to_string(&doc2).unwrap()).unwrap();
    let result = im.index_document("users", "user2", &doc2, &mut batch2);
    assert!(result.is_err(), "Should reject duplicate unique index value");
    let err = result.unwrap_err().to_string();
...
fn test_index_composite() {
    ...
    im.create_index("users_city_age_idx", "users", vec![
    ...
    for (name, doc) in &docs {
        let mut batch = WriteBatch::new();
        batch.set(&format!("users:{}", name), serde_json::to_string(doc).unwrap()).unwrap();
        im.index_document("users", name, doc, &mut batch).unwrap();
        db.commit_batch(&batch).unwrap();
    }
    
    // Lookup by (city=NYC, age=25) — should find only Alice
    let results = im.lookup("users_city_age_idx", &[serde_json::json!("NYC"), serde_json::json!(25)]).unwrap();
    assert_eq!(results.len(), 1);
    assert_eq!(results[0], "alice");
}
...
fn test_index_drop() {
    ...
    im.create_index("temp_idx", "items", vec![
        ("name".to_string(), IndexFieldType::String),
    ], false).unwrap();
    
    // Insert and index
    let doc = serde_json::json!({"name": "widget"});
    let mut batch = WriteBatch::new();
    batch.set("items:1", serde_json::to_string(&doc).unwrap()).unwrap();
    im.index_document("items", "1", &doc, &mut batch).unwrap();
    db.commit_batch(&batch).unwrap();
    
    // Verify lookup works
    let results = im.lookup("temp_idx", &[serde_json::json!("widget")]).unwrap();
    assert_eq!(results.len(), 1);
...
fn test_index_remove_document() {
    ...
    im.create_index("products_sku_idx", "products", vec![
        ("sku".to_string(), IndexFieldType::String),
    ], false).unwrap();
    
    let doc = serde_json::json!({"sku": "ABC-123"});
    let mut batch = WriteBatch::new();
    batch.set("products:1", serde_json::to_string(&doc).unwrap()).unwrap();
    im.index_document("products", "1", &doc, &mut batch).unwrap();
    db.commit_batch(&batch).unwrap();
    
    // Verify it's indexed
    let results = im.lookup("products_sku_idx", &[serde_json::json!("ABC-123")]).unwrap();
    assert_eq!(results.len(), 1);
    
    // Remove index entries
    let mut del_batch = WriteBatch::new();
    del_batch.delete("products:1").unwrap();
    im.remove_document_indexes("products", "1", &doc, &mut del_batch).unwrap();
...
fn test_index_list() {
    ...
    im.create_index("idx_a", "coll", vec![("f1".to_string(), IndexFieldType::String)], false).unwrap();
    im.create_index("idx_b", "coll", vec![("f2".to_string(), IndexFieldType::Integer)], true).unwrap();
    im.create_index("idx_c", "other", vec![("f3".to_string(), IndexFieldType::Float)], false).unwrap();
...
use omni_engine::prepared::{QueryEngine, QueryResult, PreparedAction, ParamRef};
...
fn test_prepared_insert_and_select() {
    ...
    assert!(matches!(insert_stmt.action, PreparedAction::Insert(ParamRef::Positional(1), ParamRef::Positional(2))));
...
fn test_prepared_select_range() {
    ...
    for i in 0..10 {
        let mut batch = WriteBatch::new();
        batch.set(&format!("row_{:03}", i), format!("data_{}", i)).unwrap();
        db.commit_batch(&batch).unwrap();
    }
    
    // Prepared range SELECT with params
    let stmt = qe.prepare("SELECT * WHERE key >= $1 AND key <= $2").unwrap();
...
fn test_prepared_select_count() {
    ...
    for i in 0..5 {
        let mut batch = WriteBatch::new();
        batch.set(&format!("cnt_{:03}", i), format!("v{}", i)).unwrap();
        db.commit_batch(&batch).unwrap();
    }
    
    let stmt = qe.prepare("SELECT COUNT WHERE key >= $1 AND key <= $2").unwrap();
...
fn test_prepared_update() {
    ...
    let seq = db.get_seq();
    assert_eq!(db.find("upd_key", seq).unwrap(), Some("new_value".to_string()));
}
...
fn test_named_params() {
    ...
    let result = qe.execute_named(&stmt, &[("key", "named_k"), ("value", "named_v")]).unwrap();
    assert!(matches!(result, QueryResult::Affected(1)));
    
    let seq = db.get_seq();
    assert_eq!(db.find("named_k", seq).unwrap(), Some("named_v".to_string()));
}
...
fn test_plan_cache_hits() {
    ...
    let _stmt1 = qe.prepare("SELECT * WHERE key >= $1 AND key <= $2").unwrap();
    ...
    let _stmt2 = qe.prepare("SELECT * WHERE key >= $1 AND key <= $2").unwrap();
    let (hits, misses, cached) = qe.cache_stats();
    assert_eq!(hits, 1);
    assert_eq!(misses, 1);
    assert_eq!(cached, 1);
...
fn test_select_with_limit_and_order() {
    ...
    for i in 0..10 {
        let mut batch = WriteBatch::new();
        batch.set(&format!("ord_{:03}", i), format!("v{}", i)).unwrap();
        db.commit_batch(&batch).unwrap();
    }
    
    let stmt = qe.prepare("SELECT * WHERE key >= $1 AND key <= $2 ORDER BY DESC LIMIT 3").unwrap();
...
fn create_test_schema_manager() -> (Arc<OmniKV>, Arc<IndexManager>, SchemaManager, tempfile::TempDir) {
    let dir = tempfile::TempDir::new().expect("tempdir");
    let manifest = dir.path().join("schema_manifest").to_string_lossy().to_string();
...
fn test_schema_create_index_migration() {
    ...
    for i in 0..5 {
        ...
        let mut batch = WriteBatch::new();
        batch.set(&format!("users:{}", i), serde_json::to_string(&doc).unwrap()).unwrap();
        db.commit_batch(&batch).unwrap();
    }
    ...
    let results = im.lookup("users_email_idx", &[serde_json::json!("user2@test.com")]).unwrap();
    assert_eq!(results.len(), 1);
}
...
fn test_schema_add_field_backfill() {
    ...
    for i in 0..3 {
        ...
        let mut batch = WriteBatch::new();
        batch.set(&format!("profiles:{}", i), serde_json::to_string(&doc).unwrap()).unwrap();
        db.commit_batch(&batch).unwrap();
    }
    
    // Migration: add "active" field with default true
    let m1 = add_field_migration(1, "Add active field", "profiles", "active", serde_json::json!(true));
    sm.migrate_up(&m1).unwrap();
...
fn test_schema_rename_field() {
    ...
    let mut batch = WriteBatch::new();
    batch.set("accounts:1", serde_json::to_string(&doc).unwrap()).unwrap();
    db.commit_batch(&batch).unwrap();
    
    let m1 = rename_field_migration(1, "Rename username to display_name", "accounts", "username", "display_name");
    sm.migrate_up(&m1).unwrap();
...
fn test_schema_migrate_to_target() {
    ...
    let migrations = vec![
        create_index_migration(1, "idx1", "idx_a", "coll", vec![("f1".to_string(), IndexFieldType::String)], false),
        create_index_migration(2, "idx2", "idx_b", "coll", vec![("f2".to_string(), IndexFieldType::Integer)], false),
        create_index_migration(3, "idx3", "idx_c", "coll", vec![("f3".to_string(), IndexFieldType::Float)], false),
    ];
...
fn test_schema_migration_log() {
    ...
    let m1 = create_index_migration(1, "First", "log_idx1", "c", vec![("f".to_string(), IndexFieldType::String)], false);
    let m2 = create_index_migration(2, "Second", "log_idx2", "c", vec![("g".to_string(), IndexFieldType::Integer)], false);
...
fn test_schema_migration_status() {
    ...
    let m1 = create_index_migration(1, "Status test", "status_idx", "c", vec![("f".to_string(), IndexFieldType::String)], false);
...
fn test_error_handling_no_panics() {
    ...
    let mut batch = WriteBatch::new();
    batch.set_with_ttl("ttl_test", "value".to_string(), 3600).unwrap();
    db.commit_batch(&batch).unwrap();
...
fn test_chaos_crash_recovery() {
    let dir = tempfile::TempDir::new().expect("tempdir");
    let manifest = dir.path().join("chaos_manifest").to_string_lossy().to_string();
    let wal = dir.path().join("chaos_wal").to_string_lossy().to_string();
    
    let result = chaos::test_crash_recovery(&manifest, &wal);
    assert!(result.passed, "CHAOS FAIL: {} — {}", result.test_name, result.details);
...
fn test_chaos_run_all() {
    ...
    for f in &failed {
        eprintln!("CHAOS FAILURE: {} — anomalies={} details={}", 
...
fn test_2pc_coordinator_lifecycle() {
    ...
    coord.add_write(txn_id, 2, "key_a".into(), Some("val_a".into()), 0).unwrap();
    coord.add_write(txn_id, 3, "key_b".into(), Some("val_b".into()), 0).unwrap();
...
fn test_2pc_single_node_commit() {
    ...
    let txn_id = coord.begin();
    coord.add_write(txn_id, 2, "dist_key1".into(), Some("dist_val1".into()), 0).unwrap();
...
fn test_2pc_multi_node_commit() {
    ...
    let txn_id = coord.begin();
    coord.add_write(txn_id, 2, "node2_key".into(), Some("node2_val".into()), 0).unwrap();
    coord.add_write(txn_id, 3, "node3_key".into(), Some("node3_val".into()), 0).unwrap();
    ...
    // Verify both writes
    let seq = db.get_seq();
    assert_eq!(db.find("node2_key", seq).unwrap(), Some("node2_val".to_string()));
    assert_eq!(db.find("node3_key", seq).unwrap(), Some("node3_val".to_string()));
}
...
fn test_2pc_participant_abort() {
    ...
    let txn_id = coord.begin();
    coord.add_write(txn_id, 2, "abort_key1".into(), Some("v1".into()), 0).unwrap();
    // Intentionally add a write with value too large to trigger batch error
    coord.add_write(txn_id, 3, "abort_key2".into(), Some("v2".into()), 0).unwrap();
...
fn test_2pc_coordinator_abort() {
    ...
    let txn_id = coord.begin();
    coord.add_write(txn_id, 2, "will_abort".into(), Some("nope".into()), 0).unwrap();
...
fn test_2pc_timeout_detection() {
    ...
    let txn_id = coord.begin();
    coord.add_write(txn_id, 2, "timeout_key".into(), Some("v".into()), 0).unwrap();
    coord.prepare(txn_id).unwrap();
...
fn test_2pc_prepared_tracking() {
    ...
    let writes1 = vec![DistWrite {
        node_id: 1, key: "prep1".into(), value: Some("v1".into()), ttl: 0,
    }];
    let writes2 = vec![DistWrite {
        node_id: 1, key: "prep2".into(), value: Some("v2".into()), ttl: 0,
    }];
...
fn test_2pc_recovery_log() {
    ...
    let txn_id = coord.begin();
    coord.add_write(txn_id, 2, "recovery_key".into(), Some("recovery_val".into()), 0).unwrap();
    coord.prepare(txn_id).unwrap();
